...
struct wlfw_cnss_request_mem_ind_msg_v01 ind_message = {0};
...
{
	struct wlfw_cnss_ind_register_req_msg_v01 *req;
	struct wlfw_cnss_ind_register_resp_msg_v01 *resp;
	struct qmi_txn txn;
	int ret = 0;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_ind_register_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_ind_register_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
{
	struct wlfw_cnss_host_cap_req_msg_v01 *req;
	struct wlfw_cnss_host_cap_resp_msg_v01 *resp;
	struct qmi_txn txn;
	int ret = 0;
	int resp_error_msg = 0;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_host_cap_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_host_cap_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
{
	struct wlfw_cnss_respond_mem_req_msg_v01 *req;
	struct wlfw_cnss_respond_mem_resp_msg_v01 *resp;
	struct qmi_txn txn;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_respond_mem_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_respond_mem_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
{
	struct wlfw_cnss_cap_req_msg_v01 *req;
	struct wlfw_cnss_cap_resp_msg_v01 *resp;
	struct qmi_txn txn;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_cap_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_cap_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
static int cnss_wlfw_load_bdf(struct wlfw_cnss_bdf_download_req_msg_v01 *req,
				 ...
				 u32 bdf_type)
{
	...
	struct wlfw_cnss_bdf_download_req_msg_v01 *req;
	struct wlfw_cnss_bdf_download_resp_msg_v01 *resp;
	int ret = 0;
	int resp_error_msg = 0;
...
bypass_bdf:
	...
	while (remaining) {
		...
		ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
				   wlfw_cnss_bdf_download_resp_msg_v01_ei, resp);
		if (ret < 0) {
		...
		ret = qmi_send_request
			(&plat_priv->qmi_wlfw, NULL, &txn,
			 ...
			 wlfw_cnss_bdf_download_req_msg_v01_ei, req);
		if (ret < 0) {
			qmi_txn_cancel(&txn);
...
{
	struct wlfw_cnss_m3_info_req_msg_v01 *req;
	struct wlfw_cnss_m3_info_resp_msg_v01 *resp;
	struct qmi_txn txn;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_m3_info_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_m3_info_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
{
	struct wlfw_cnss_wlan_mode_req_msg_v01 *req;
	struct wlfw_cnss_wlan_mode_resp_msg_v01 *resp;
	struct qmi_txn txn;
	int ret = 0;
	int resp_error_msg = 0;
	...
	req->mode = (enum wlfw_cnss_driver_mode_enum_v01)mode;
	req->hw_debug_valid = 1;
	req->hw_debug = 0;
	qmi_record(plat_priv->wlfw_service_instance_id,
		   QMI_WLFW_WLAN_MODE_REQ_V01, ret, resp_error_msg);
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_wlan_mode_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_wlan_mode_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
{
	struct wlfw_cnss_wlan_cfg_req_msg_v01 *req;
	struct wlfw_cnss_wlan_cfg_resp_msg_v01 *resp;
	struct qmi_txn txn;
	u32 i;
	int ret = 0;
	int resp_error_msg = 0;
	...
	memcpy(req->shadow_reg_v2, config->shadow_reg_v2_cfg,
	       sizeof(struct wlfw_cnss_shadow_reg_v2_cfg_s_v01)
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_wlan_cfg_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_wlan_cfg_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
{
	struct wlfw_cnss_athdiag_read_req_msg_v01 *req;
	struct wlfw_cnss_athdiag_read_resp_msg_v01 *resp;
	struct qmi_txn txn;
	int ret = 0;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_athdiag_read_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_athdiag_read_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
int cnss_wlfw_athdiag_write_send_sync(struct cnss_plat_data *plat_priv,
				      ...
				      u32 data_len, u8 *data)
{
	struct wlfw_cnss_athdiag_write_req_msg_v01 *req;
	struct wlfw_cnss_athdiag_write_resp_msg_v01 *resp;
	struct qmi_txn txn;
	int ret = 0;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_athdiag_write_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_athdiag_write_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
int cnss_wlfw_ini_send_sync(struct cnss_plat_data *plat_priv,
			    u8 fw_log_mode)
{
	struct wlfw_cnss_ini_req_msg_v01 *req;
	struct wlfw_cnss_ini_resp_msg_v01 *resp;
	struct qmi_txn txn;
	int ret = 0;
	int resp_error_msg = 0;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_ini_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_ini_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
{
	struct wlfw_cnss_antenna_switch_req_msg_v01 *req;
	struct wlfw_cnss_antenna_switch_resp_msg_v01 *resp;
	struct qmi_txn txn;
	int ret = 0;
	int resp_error_msg = 0;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_antenna_switch_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_antenna_switch_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
{
	struct wlfw_cnss_antenna_grant_req_msg_v01 *req;
	struct wlfw_cnss_antenna_grant_resp_msg_v01 *resp;
	struct qmi_txn txn;
	int ret = 0;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_antenna_grant_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_antenna_grant_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
{
	struct wlfw_cnss_qdss_trace_mem_info_req_msg_v01 *req;
	struct wlfw_cnss_qdss_trace_mem_info_resp_msg_v01 *resp;
	struct qmi_txn txn;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_qdss_trace_mem_info_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_qdss_trace_mem_info_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
{
	struct wlfw_cnss_wfc_call_status_req_msg_v01 *req;
	struct wlfw_cnss_wfc_call_status_resp_msg_v01 *resp;
	struct qmi_txn txn;
	int ret = 0;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_wfc_call_status_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_wfc_call_status_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
{
	struct wlfw_cnss_dynamic_feature_mask_req_msg_v01 *req;
	struct wlfw_cnss_dynamic_feature_mask_resp_msg_v01 *resp;
	struct qmi_txn txn;
	int ret = 0;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_dynamic_feature_mask_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request
		(&plat_priv->qmi_wlfw, NULL, &txn,
		 ...
		 wlfw_cnss_dynamic_feature_mask_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
{
	struct wlfw_cnss_m3_dump_upload_done_req_msg_v01 *req;
	struct wlfw_cnss_m3_dump_upload_done_resp_msg_v01 *resp;
	struct qmi_txn txn;
	int ret = 0;
	int resp_error_msg = 0;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_m3_dump_upload_done_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_m3_dump_upload_done_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
...
{
	struct wlfw_cnss_device_info_req_msg_v01 *req;
	struct wlfw_cnss_device_info_resp_msg_v01 *resp;
	struct qmi_txn txn;
	int ret = 0;
	int resp_error_msg = 0;
	...
	ret = qmi_txn_init(&plat_priv->qmi_wlfw, &txn,
			   wlfw_cnss_device_info_resp_msg_v01_ei, resp);
	if (ret < 0) {
	...
	ret = qmi_send_request(&plat_priv->qmi_wlfw, NULL, &txn,
			       ...
			       wlfw_cnss_device_info_req_msg_v01_ei, req);
	if (ret < 0) {
		qmi_txn_cancel(&txn);
					 ...
					 struct qmi_txn *txn, const void *data)
{
	...
	const struct wlfw_cnss_request_mem_ind_msg_v01 *ind_msg = data;
	int i;
					...
					struct qmi_txn *txn, const void *data)
{
	struct cnss_plat_data *plat_priv =
		container_of(qmi_wlfw, struct cnss_plat_data, qmi_wlfw);
	const struct wlfw_cnss_pin_connect_result_ind_msg_v01 *ind_msg = data;
						...
						const void *data)
{
	struct cnss_plat_data *plat_priv =
		container_of(qmi_wlfw, struct cnss_plat_data, qmi_wlfw);
	const struct wlfw_cnss_qdss_trace_req_mem_ind_msg_v01 *ind_msg = data;
	int i;
					     ...
					     const void *data)
{
	struct cnss_plat_data *plat_priv =
		container_of(qmi_wlfw, struct cnss_plat_data, qmi_wlfw);
	const struct wlfw_cnss_qdss_trace_save_ind_msg_v01 *ind_msg = data;
						...
						const void *data)
{
	struct cnss_plat_data *plat_priv =
		container_of(qmi_wlfw, struct cnss_plat_data, qmi_wlfw);
	const struct wlfw_cnss_m3_dump_upload_req_ind_msg_v01 *ind_msg = data;
...
static struct qmi_msg_handler qmi_wlfw_msg_handlers[] = {
	{
		...
		.ei = wlfw_cnss_request_mem_ind_msg_v01_ei,
		.decoded_size = sizeof(struct wlfw_cnss_request_mem_ind_msg_v01),
		.fn = cnss_wlfw_request_mem_ind_cb,
	},
	{
		.type = QMI_INDICATION,
		.msg_id = QMI_WLFW_FW_MEM_READY_IND_V01,
		.ei = wlfw_cnss_fw_mem_ready_ind_msg_v01_ei,
		.decoded_size = sizeof(struct wlfw_cnss_fw_mem_ready_ind_msg_v01),
		.fn = cnss_wlfw_fw_mem_ready_ind_cb,
	},
	{
		.type = QMI_INDICATION,
		.msg_id = QMI_WLFW_FW_READY_IND_V01,
		.ei = wlfw_cnss_fw_ready_ind_msg_v01_ei,
		.decoded_size = sizeof(struct wlfw_cnss_fw_ready_ind_msg_v01),
		.fn = cnss_wlfw_fw_ready_ind_cb,
	},
	{
		.type = QMI_INDICATION,
		.msg_id = QMI_WLFW_FW_INIT_DONE_IND_V01,
		.ei = wlfw_cnss_fw_init_done_ind_msg_v01_ei,
		.decoded_size = sizeof(struct wlfw_cnss_fw_init_done_ind_msg_v01),
		.fn = cnss_wlfw_fw_init_done_ind_cb,
	},
	{
		.type = QMI_INDICATION,
		.msg_id = QMI_WLFW_PIN_CONNECT_RESULT_IND_V01,
		.ei = wlfw_cnss_pin_connect_result_ind_msg_v01_ei,
		.decoded_size =
			sizeof(struct wlfw_cnss_pin_connect_result_ind_msg_v01),
		.fn = cnss_wlfw_pin_result_ind_cb,
	},
	{
		.type = QMI_INDICATION,
		.msg_id = QMI_WLFW_CAL_DONE_IND_V01,
		.ei = wlfw_cnss_cal_done_ind_msg_v01_ei,
		.decoded_size = sizeof(struct wlfw_cnss_cal_done_ind_msg_v01),
		.fn = cnss_wlfw_cal_done_ind_cb
	},
	{
		.type = QMI_INDICATION,
		.msg_id = QMI_WLFW_QDSS_TRACE_REQ_MEM_IND_V01,
		.ei = wlfw_cnss_qdss_trace_req_mem_ind_msg_v01_ei,
		.decoded_size =
			sizeof(struct wlfw_cnss_qdss_trace_req_mem_ind_msg_v01),
		.fn = cnss_wlfw_qdss_trace_req_mem_ind_cb,
	},
	{
		.type = QMI_INDICATION,
		.msg_id = QMI_WLFW_QDSS_TRACE_SAVE_IND_V01,
		.ei = wlfw_cnss_qdss_trace_save_ind_msg_v01_ei,
		.decoded_size =
			sizeof(struct wlfw_cnss_qdss_trace_save_ind_msg_v01),
		.fn = cnss_wlfw_qdss_trace_save_ind_cb,
	},
	{
		.type = QMI_INDICATION,
		.msg_id = QMI_WLFW_QDSS_TRACE_FREE_IND_V01,
		.ei = wlfw_cnss_qdss_trace_free_ind_msg_v01_ei,
		.decoded_size =
			sizeof(struct wlfw_cnss_qdss_trace_free_ind_msg_v01),
		.fn = cnss_wlfw_qdss_trace_free_ind_cb,
	},
	{
		.type = QMI_INDICATION,
		.msg_id = QMI_WLFW_M3_DUMP_UPLOAD_REQ_IND_V01,
		.ei = wlfw_cnss_m3_dump_upload_req_ind_msg_v01_ei,
		.decoded_size =
			sizeof(struct wlfw_cnss_m3_dump_upload_req_ind_msg_v01),
