	...
	bool                      *send2ipacm)
{
	...
	if (eogre_info == NULL || send2uC == NULL || send2ipacm == NULL) {
		IPAERR("NULL ptr: eogre_info(%pK)", eogre_info);
		IPAERR("and/or send2uC(%pK) and/or send2ipacm(%pK)\n",
	...
	*send2uC = *send2ipacm = false;
	...
	if (cache_is_null) {
		...
		if (eogre_is_null) {
			IPAERR("Attempting to disable EoGRE.");
			IPAERR("EoGRE already disabled.No work to be done.\n");
			ret = -EIO;
			goto done;
		}
		...
		*send2uC = *send2ipacm = true;
		...
		if (!eogre_is_null) {
			...
			IPAERR("No work needs to be done.\n");
			ret = -EIO;
			goto done;
		}
